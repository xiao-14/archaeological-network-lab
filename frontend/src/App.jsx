...
function App() {
  ...
  useEffect(() => {
    ...
    const visNodes = bnodes.map(n => n.type === 'burial'
    ...
    const visEdges = bedges.map(e => ({ from: e.from, to: e.to, color: { color: '#cccccc', opacity: 0.5 }, width: 0.5 }));
    ...
    <section className="panel" style={{marginTop:18}}>
      ...
      {attrAnalysis && <>
        ...
        <div style={{marginTop:16, padding:16, background:'#f8f6f2', borderRadius:14, border:'1px solid #e0d8cc'}}>
          ...
          <div ref={bipartiteRef} style={{height:500, background:'white', borderRadius:10, border:'1px solid #dedede'}}></div>
        </div>
      </>}
    </section>
  </main>;
