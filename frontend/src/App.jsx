    ...
    <section className="panel" style={{marginTop:18}}>
      ...
      {attrAnalysis && <>
        ...
        <div style={{marginTop:16, padding:16, background:'#f8f6f2', borderRadius:14, border:'1px solid #e0d8cc'}}>
          ...
          <div ref={bipartiteWrapRef} style={{background:'white', borderRadius:14, border:'1px solid #dedede', overflow:'hidden'}}>
            <div style={{padding:'16px 18px 8px'}}>
              <div style={{fontSize:18, fontWeight:700, marginBottom:6}}>{t.bipartiteTitle}</div>
              <div style={{fontSize:12, color:'#666', marginBottom:8}}>
                {t.bipartiteInfo(attrAnalysis.bipartite.burial_count, attrAnalysis.bipartite.artifact_count)}
                {bipartiteFilterValue && <> — {t.bipartiteFilterLabel}: <b>{bipartiteFilterValue}</b></>}
              </div>
              {/* 图例：实际颜色映射 */}
              <div style={{display:'flex', gap:14, flexWrap:'wrap', alignItems:'center', fontSize:12, color:'#444', marginBottom:6}}>
                <b style={{fontSize:12}}>{attrAnalysis.attr_col}:</b>
                {bipartiteAttrValues.map((v, i) => {
                  const OKABE = ["#E69F00","#56B4E9","#009E73","#F0E442","#0072B2","#D55E00","#CC79A7","#999999","#000000","#44AA99"];
                  return (
                    <span key={v} style={{ display:"inline-flex", alignItems:"center", gap:4 }}>
                      <span style={{ width:11, height:11, borderRadius:"50%", background:OKABE[i % OKABE.length], display:"inline-block", border:"1px solid #ccc" }}></span>
                      {v}
                    </span>
                  );
                })}
                <span style={{borderLeft:"1px solid #ddd", height:14}}/>
                <span style={{display:'inline-flex', alignItems:'center', gap:5}}>
                  <svg width="14" height="14"><rect x="1" y="1" width="12" height="12" fill="#e8e0d0" stroke="#b0a080" strokeWidth="1"/></svg>
                  {lang === 'zh' ? '器物类型（大小=出现频率）' : 'Artifact type (size = frequency)'}
                </span>
              </div>
            </div>

            <div style={{ position:"relative" }}>
              <div ref={bipartiteRef} style={{height:500, borderTop:'1px solid #eee'}}></div>
              <div style={{ position:"absolute", right:14, bottom:14, background:"rgba(255,255,255,0.92)", border:"1px solid #d9d9d9", borderRadius:10, padding:"8px 12px", fontSize:11, lineHeight:1.6, boxShadow:"0 2px 8px rgba(0,0,0,0.08)" }}>
                <div><b>{t.attrAnalysisCol}</b>: {attrAnalysis.attr_col}</div>
                <div><b>{t.bipartiteFilterLabel}</b>: {bipartiteFilterValue || t.bipartiteFilterAll}</div>
              </div>
            </div>
          </div>
        </div>
