...
@app.post("/api/attribute_analysis")
async def attribute_analysis(
    file: UploadFile = File(...),
    method: Method = Form("jaccard"),
    threshold: float = Form(0.3),
    top_k: int = Form(5),
    attr_col: str = Form("性别"),
):
    """
    属性分析模块（通用考古学网络平台）：
    1. 网络同质性检验 (assortativity) — 检验该属性是否驱动网络连接
    2. 按属性分组的网络中心性统计 — 各组的度/中介中心性均值与分布
    3. 二模网络数据 — 墓葬节点 + 器物类型节点 + 关联边，供前端可视化
    """
    content = await file.read()
    df = read_table(file, content)
    data = prepare_data(df)
    ...
    G = build_artifact_network(data.matrix, method, threshold, top_k, binary=True)
    if G.number_of_edges() == 0:
        raise HTTPException(status_code=422, detail="当前参数无连接边，无法进行属性分析。")

    node_list = [str(x) for x in G.nodes()]

    # ── 1. 同质性检验 ──────────────────────────────────────────────
    attr_map = {}
    for n in node_list:
        val = data.attrs.get(n, {}).get(attr_col)
        if val is not None and not (isinstance(val, float) and math.isnan(val)) and str(val).strip() not in ("", "nan"):
            attr_map[n] = str(val).strip()

    coverage = round(len(attr_map) / max(len(node_list), 1), 4)
    assortativity_result: Dict[str, Any] = {"coverage": coverage}
    try:
        nx.set_node_attributes(G, attr_map, name="_attr_")
        numeric_map = {}
        for n, v in attr_map.items():
            try:
                numeric_map[n] = float(v)
            except ValueError:
                pass
        if len(numeric_map) == len(attr_map) and len(numeric_map) > 1:
            nx.set_node_attributes(G, numeric_map, name="_attr_num_")
            r = nx.numeric_assortativity_coefficient(G, "_attr_num_")
            assortativity_result.update({"type": "numeric", "r": round(float(r), 4),
                "interpretation": _interpret_assortativity(float(r), attr_col, is_numeric=True)})
        else:
            sub = G.subgraph([n for n in G.nodes() if n in attr_map]).copy()
            if sub.number_of_edges() > 0:
                r = nx.attribute_assortativity_coefficient(sub, "_attr_")
                assortativity_result.update({"type": "categorical", "r": round(float(r), 4),
                    "interpretation": _interpret_assortativity(float(r), attr_col, is_numeric=False)})
            else:
                assortativity_result.update({"type": "categorical", "r": None, "interpretation": "属性覆盖节点间无连接边，无法计算同质性。"})
    except Exception as e:
        assortativity_result.update({"type": "error", "r": None, "interpretation": str(e)})
    ...
    for u, v, d in H.edges(data=True):
        d["distance"] = 1 / max(float(d.get("weight", 1e-9)), 1e-9)
    between_c = nx.betweenness_centrality(H, weight="distance", normalized=True)
    clustering_c = nx.clustering(G, weight="weight")

    groups: Dict[str, Dict[str, Any]] = {}
    for n in node_list:
        grp = attr_map.get(n, "unknown")
        if grp not in groups:
            groups[grp] = {"degree": [], "betweenness": [], "clustering": [], "node_count": 0}
        groups[grp]["degree"].append(degree_c.get(n, 0))
        groups[grp]["betweenness"].append(between_c.get(n, 0))
        groups[grp]["clustering"].append(clustering_c.get(n, 0))
        groups[grp]["node_count"] += 1

    def _stats(lst: List[float]) -> Dict[str, float]:
        a = np.array(lst)
        return {"mean": round(float(a.mean()), 4), "median": round(float(np.median(a)), 4),
                "std": round(float(a.std()), 4), "max": round(float(a.max()), 4)}

    centrality_by_group = {
        grp: {"node_count": vals["node_count"], "degree": _stats(vals["degree"]),
              "betweenness": _stats(vals["betweenness"]), "clustering": _stats(vals["clustering"])}
        for grp, vals in groups.items()
    }

    # ── 3. 二模网络数据 ──────────────────────────────────────────────
    artifact_cols = [str(c) for c in data.matrix.columns]
    bipartite_nodes = []
    bipartite_edges = []
    artifact_degree = (data.matrix > 0).sum(axis=0)

    for n in data.matrix.index:
        ns = str(n)
        bipartite_nodes.append({"id": f"b_{ns}", "label": ns, "type": "burial",
                                 "attr_val": attr_map.get(ns, "unknown"), "shape": "dot"})
    for ac in artifact_cols:
        bipartite_nodes.append({"id": f"a_{ac}", "label": ac, "type": "artifact",
                                 "attr_val": "artifact", "size": int(artifact_degree.get(ac, 1))})
    for n in data.matrix.index:
        ns = str(n)
        row = data.matrix.loc[n]
        for ac in artifact_cols:
            if row[ac] > 0:
                bipartite_edges.append({"from": f"b_{ns}", "to": f"a_{ac}", "weight": float(row[ac])})

    result = {
        "attr_col": attr_col,
        "assortativity": assortativity_result,
        "centrality_by_group": centrality_by_group,
        "bipartite": {
            "nodes": bipartite_nodes, "edges": bipartite_edges,
            "burial_count": len(data.matrix.index), "artifact_count": len(artifact_cols),
        },
        "note": ("同质性系数 r 接近 +1 表示相同属性值的节点倾向互相连接（同质聚集）；"
                 "r 接近 -1 表示不同属性值节点更多互连（异质桥接）；"
                 "r 接近 0 表示该属性对网络结构无显著影响。")
    }
    return clean_for_json(result)


def _interpret_assortativity(r: float, attr_col: str, is_numeric: bool) -> str:
    if r is None:
        return "无法计算。"
    kind = "数值属性" if is_numeric else "分类属性"
    if r > 0.3:
        return f"强正同质性（r={r:.3f}）：具有相同'{attr_col}'值的墓葬倾向于共享相似器物组合，该{kind}是网络聚集的重要驱动因素。"
    if r > 0.1:
        return f"弱正同质性（r={r:.3f}）：'{attr_col}'对网络连接有一定影响，但不是主要驱动因素。"
    if r < -0.3:
        return f"强负同质性（r={r:.3f}）：不同'{attr_col}'值的墓葬更多互连，可能存在跨群体的器物交流或陪葬习俗互动。"
    if r < -0.1:
        return f"弱负同质性（r={r:.3f}）：'{attr_col}'略微倾向于异质连接。"
    return f"中性（r={r:.3f}）：'{attr_col}'对网络结构无显著影响，器物组合相似性与该属性关联不强。"
